...
class WaypointMarkerPublisher(Node):
    def __init__(self):
        ...
        # Load waypoints - MUST match the file used in control.py
        self.load_waypoints("/home/avinash0104/f1tenth_ws/src/control/8_track_shifted.csv")
    ...
    def load_waypoints(self, filename):
        ...
        try:
            ...
            self.get_logger().info(f"Successfully loaded {len(self.waypoints)} waypoints from {filename}")
    ...
    def timer_callback(self):
        ...
        for i, (x, y, heading) in enumerate(self.waypoints):
            ...
            marker.scale.x = 0.5  # Make arrows much larger and visible
            marker.scale.y = 0.1  
            marker.scale.z = 0.1  
        ...
        if self.publish_count % 10 == 1:
            self.get_logger().info(f"Publishing {len(marker_array.markers)} markers to /waypoints_markers")
